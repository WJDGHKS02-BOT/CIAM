//정지수 test파일 생성, 원형은 search_audit_log.js 파일로부터
// ag-grid object
let oSearchRevisionListResultGrid = null;
...
function clearRevisionListGrid(){
    if (!!currentGrid) {
        currentGrid.destroy();
    }
}
...
// clear grid data
function initializeRevisionListGrid() {
    resetFilters();

    doSearchRevisionList();
}
...
function doSearchRevisionList(event) {
    if (!!currentGrid) {
    ...
    $.ajax({
        url: "/search/revisionList",
        type: "POST",
        data: JSON.stringify(params),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (oResult) {
            renderRevisionList(oResult);
...
function renderRevisionList(oResult) {
