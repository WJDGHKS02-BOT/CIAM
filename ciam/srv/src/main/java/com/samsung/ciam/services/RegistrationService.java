  ...
  private void updateCdcCompany(Map<String, String> requestParams, List<String> industryArray) {
    try {
      ...
      if (response.getErrorCode() == 0) {
        ...
        log.error("Failed to update CDC company: cdc error message -> " + response.getErrorMessage() + " cdc error detail message -> " + response.getErrorDetails());
      }
    } catch (Exception e) {
        ...
        if (response.getErrorCode() == 0) {
          ...
          try {
            ...
            if (otpResponse.getErrorCode() == 0) {
              ...
              String ciamCountry = CDCUser.path("profile").path("country").asText("");

              //sec_country테이블 ID로 국가코드 조회
              Long countryCode = Long.parseLong(payload.get("country"));
              Optional<SecServingCountry> secCountryOpt = secServingCountryRepository.findByCountryCode(countryCode);
              SecServingCountry secCountry = secCountryOpt.get();
              String requestCountry = secCountry.getCountryCode();

              //jcext 전환확장시 국가정보 일치하지않을시 에러문구 발생
              if (ciamCountry != null && requestCountry != null && !ciamCountry.isEmpty() && !requestCountry.isEmpty() && !Objects.equals(ciamCountry, requestCountry)) {
                //국가코드로 국가명 조회
                String countryName = cisCountryRepository.findNameEnByCountryCode(ciamCountry);

                // 'countryName' 변수를 이용하여 메시지 생성 (국가명)
                String errorMessage = String.format("The country in your CIAM account does not match.\nThe country currently registered in CIAM is\n%s (%s).", countryName, ciamCountry);

                // 오류 메시지 설정
                redirectAttributes.addFlashAttribute("showErrorMsg", errorMessage);
                //페이지 이동
                return new RedirectView(request.getHeader("Referer"));
              }

              //전환확장시 회사코드 일치하지 않을경우 에러 문구 발생
              if (companyId != null && accountId != null && !companyId.isEmpty() && !accountId.isEmpty() && !Objects.equals(companyId, accountId)) {
                ...
                // 전환확장시 ACCOUND ID가 채널측에서 안들어왔을시 CIAM 회사코드로 저장
...
//                }
        ...
        if (btpAccount != null) {
          try {
            ...
            String streetAddress = btpAccount.getStreetAddress() != null ? btpAccount.getStreetAddress().toLowerCase() : "";
            organizationMap.put("street_address", streetAddress);
