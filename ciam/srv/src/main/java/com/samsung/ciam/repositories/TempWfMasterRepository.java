...
import com.samsung.ciam.models.TempWfMaster;
...
public interface TempWfMasterRepository extends JpaRepository<TempWfMaster, String> {

    // 현재 날짜에 해당하는 wf_id의 최대값을 가져오는 쿼리
    @Query("SELECT MAX(w.wfId) FROM TempWfMaster w WHERE w.wfId LIKE CONCAT(:prefix, '%')")
    ...
    @Transactional
    @Modifying
    @Query(value =  "INSERT INTO temp_wf_master ( wf_id, channel, workflow_code, requestor_id, requestor_email, requestor_role, status," +
    ...
    @Transactional
    @Modifying
    @Query(value =  "INSERT INTO temp_wf_list ( wf_id, workflow_code, wf_level, status, approval_format, approver_id, approver_email, approver_role, approved_date, approved_id, approved_email ) " +
            "VALUES ( " +
    ...
    @Query(value = "SELECT wl.id, wl.wf_id, wl.workflow_code, wl.status, wm.channel, wm.requestor_id, wm.requestor_email, wm.requestor_company_name, wm.requestor_company_code, " +
            ...
            "FROM temp_wf_list wl " +
            "JOIN temp_wf_master wm ON wl.wf_id = wm.wf_id " +
    ...
    @Query(value = "SELECT rm.id, rm.wf_id, rm.bpid, rm.bp_name, rm.change_role, rm.requestor_uid, rm.requestor_email, rm.channel, " +
            ...
            " FROM temp_wf_list wl JOIN role_management rm ON wl.wf_id = rm.wf_id " +
    ...
    @Query(value = "SELECT vd.id, vd.wf_id, vd.bpid, vd.bp_name, vd.email_domain, vd.requestor_uid, vd.requestor_email, " +
            ...
            ") AS company_domain " +
            " FROM temp_wf_list wl JOIN verification_domains vd ON wl.wf_id = vd.wf_id " +
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE temp_wf_list " +
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE temp_wf_master " +
    ...
    @Query(value = "SELECT * FROM temp_wf_master wm WHERE wm.wf_id = :wfId LIMIT 1", nativeQuery = true)
    TempWfMaster selectWfMaster(@Param("wfId") String wfId);
}
