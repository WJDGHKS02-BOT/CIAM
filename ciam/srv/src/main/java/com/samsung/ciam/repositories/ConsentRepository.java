...
public interface ConsentRepository extends JpaRepository<Consent, Long> {
    ...
    @Query(value = "select distinct language_id from consents c inner join consent_contents cc on c.id = cc.consent_id where cc.id < 2000 and c.type_id = :typeId and c.coverage = :coverage and c.countries LIKE '%'||:countries||'%' ORDER BY language_id", nativeQuery = true)
